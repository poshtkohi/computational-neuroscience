...
function [dxdt] = reaction_network_hp2y12(t, x, opt)
    ...
    if c.periodic == 0
        ...
        %[opt.ADP_t1 opt.ADP_t2 ADP]
    else
        %opt.amplitude = 50;
        %[ADP, G] = periodic_function(t, opt.delay, opt.width, opt.amplitude);
        [ADP, G] = periodic_function(t, opt.ADP_t1, opt.ADP_t2, opt.delay, opt.amplitude);
    end
