...
function p2x_cai_prediction()
    ...
    format long g;
    %ATP = [0.01e-3 0.1e-3 0.5e-3 1.0e-3 3.0e-3];
    ATP = [0.01e-3 0.05e-3 0.5e-3 1.0e-3 3.0e-3];
    ...
    %ATP_t1 = 1 * 60; ATP_t2 = 30*60; t_max = 35 * 60;% unit in seconds
	...
	%opt.initial_condition_index = length(k_rp2x4_cai) + 1;
	%opt.x0 = load_initial_conditions_from_k_rp2x4_total_current(K_rp2x4_cai, opt);
	%opt.x0
	%K_rp2x4_cai
	...
	%odeopt = odeset;%('Iacobian', @compute_Iacobian_matrix_rp2x4_total_current);
    ...
    for i=1:1:length(ATP)
        opt.ATP = ATP(i);
        my_ATP = ATP(i);
        %sol_p2x_cai(i) = ode15s(@(t, x)(reaction_network_p2x_cai_prediction(t, x, opt)), opt.tspan, opt.x0, odeopt);
    ...
    %i_tot_rp2x4 = y_p2x_cai(6, :) * (7.0 / 6.2) * 372;
    %i_tot = i_tot_hp2x7 + i_tot_rp2x4;
	hold on;
    ...
    for i=1:1:length(cai1)
        ...
        I_rP2X4_Na1(i) = 0.12 * 6.2 * O21(i) * (Vm1(i) - E_Na1(i)) * 1e3 * 1e-12/ s_tot/d;    % 0.38
        I_rP2X4_Na2(i) = 0.12 * 6.2 * O22(i) * (Vm2(i) - E_Na2(i)) * 1e3 * 1e-12/ s_tot/d;    % 0.38
        I_rP2X4_Na3(i) = 0.12 * 6.2 * O23(i) * (Vm3(i) - E_Na3(i)) * 1e3 * 1e-12/ s_tot/d;    % 0.38
        I_rP2X4_Na4(i) = 0.12 * 6.2 * O24(i) * (Vm4(i) - E_Na4(i)) * 1e3 * 1e-12/ s_tot/d;    % 0.38
        I_rP2X4_Na5(i) = 0.12 * 6.2 * O25(i) * (Vm5(i) - E_Na5(i)) * 1e3 * 1e-12/ s_tot/d;    % 0.38
        
        I_rP2X4_Ca1(i) = 0.1 * 6.2 * O21(i) * (Vm1(i) - E_Ca1(i)) * 1e3 * 1e-12/ s_tot/d;      % 0.1
        I_rP2X4_Ca2(i) = 0.1 * 6.2 * O22(i) * (Vm2(i) - E_Ca2(i)) * 1e3 * 1e-12/ s_tot/d;      % 0.1
        I_rP2X4_Ca3(i) = 0.1 * 6.2 * O23(i) * (Vm3(i) - E_Ca3(i)) * 1e3 * 1e-12/ s_tot/d;      % 0.1
        I_rP2X4_Ca4(i) = 0.1 * 6.2 * O24(i) * (Vm4(i) - E_Ca4(i)) * 1e3 * 1e-12/ s_tot/d;      % 0.1
        I_rP2X4_Ca5(i) = 0.1 * 6.2 * O25(i) * (Vm5(i) - E_Ca5(i)) * 1e3 * 1e-12/ s_tot/d;      % 0.1
    end
    ...
    plot(tint, I_rP2X4_Na2 * s_tot * 1e12, '-.r*', 'MarkerIndices', 1:steps:length(I_rP2X4_Na2));
	...
	hold off;
    legend('I_{NaNCX}', 'I_{NaLeak}', 'I_{hP2X7}_{Na}', 'I_{rP2X4}_{Na}', 'FontSize', 8, 'FontWeight', 'bold');
    %legend('I_{NaNCX}', 'I_{NaLeak}', 'I_{hP2X7}_{Na}', 'FontSize', 8, 'FontWeight', 'bold');
    %legend('I_{NaNCX}', 'I_{NaLeak}', 'I_{rP2X4}_{Na}', 'FontSize', 8, 'FontWeight', 'bold');
    ...
    plot(tint, I_rP2X4_Na4 * s_tot * 1e12, '-.r*', 'MarkerIndices', 1:steps:length(I_rP2X4_Na4));
    plot([ATP_t1/1 ATP_t2/1], [0.5 0.5], '-k');% , 'LineWidth', 2);
    ...
    plot(tint, I_rP2X4_Na5 * s_tot * 1e12, '-.r*', 'MarkerIndices', 1:steps:length(I_rP2X4_Na5));
    plot([ATP_t1/1 ATP_t2/1], [0.5 0.5], '-k');% , 'LineWidth', 2);
    ...
    plot(tint, I_rP2X4_Ca2 * s_tot * 1e12, '-.r*', 'MarkerIndices', 1:steps:length(I_rP2X4_Ca2));
    plot([ATP_t1/1 ATP_t2/1], [0.5 0.5], '-k');% , 'LineWidth', 2);
    text((ATP_t1 + (ATP_t2 - ATP_t1)/2), 0.8, sprintf('ATP=%gmM', ATP(2)), 'FontSize', 8);
	hold off;
    legend('I_{CaNCX}', 'I_{PMCA}', 'I_{CaLeak}', 'I_{hP2X7}_{Ca}', 'I_{rP2X4}_{Ca}', 'FontSize', 8, 'FontWeight', 'bold');
    %legend('I_{CaNCX}', 'I_{PMCA}', 'I_{CaLeak}', 'I_{hP2X7}_{Ca}', 'FontSize', 8, 'FontWeight', 'bold');
    %legend('I_{CaNCX}', 'I_{PMCA}', 'I_{CaLeak}', 'I_{rP2X4}_{Ca}', 'FontSize', 8, 'FontWeight', 'bold');
    ...
    plot(tint, I_rP2X4_Ca4 * s_tot * 1e12, '-.r*', 'MarkerIndices', 1:steps:length(I_rP2X4_Ca4));
    plot([ATP_t1/1 ATP_t2/1], [0.5 0.5], '-k');% , 'LineWidth', 2);
    ...
    plot(tint, I_rP2X4_Ca5 * s_tot * 1e12, '-.r*', 'MarkerIndices', 1:steps:length(I_rP2X4_Ca5));
    plot([ATP_t1/1 ATP_t2/1], [0.5 0.5], '-k');% , 'LineWidth', 2);
    ...
    %legend('Original shape', '1% perturbation of the original shape', 'FontWeight', 'bold');
