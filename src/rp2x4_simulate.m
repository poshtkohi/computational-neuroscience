...
function rp2x4_simulate()
    ...
    ATP_t1 = 2; ATP_t2 = 14; t_max = 16;% unit in seconds
    
    global nT_P2X T;
    T = 0; % Period in seconds
    nT_P2X = 0;

    es_rp2x4_i_tot = load('variablescmaes-rp2x4-ATP-1.000000e-04-popsize-18-total-current.mat');
	K_rp2x4_i_tot = es_rp2x4_i_tot.out.solutions.bestever.x;
    %K_rp2x4_i_tot = [1.523384e+05 4.679995e+03 4.149279e+02 6.874058e-01 1.942445e+02 5.770408e+01 7.289234e-01 3.116321e+03 1.598147e+00 2.494154e+07 ];
    %K_rp2x4_i_tot = read_last_line();
	k_rp2x4_i_tot = K_rp2x4_i_tot;%(1:7);
    k = k_rp2x4_i_tot ;
	opt_rp2x4_i_tot.x0 = zeros(4, 1);
    opt_rp2x4_i_tot.x0(1, 1) = 1;
	opt_rp2x4_i_tot.initial_condition_index = length(k_rp2x4_i_tot) + 1;
	opt_rp2x4_i_tot.x0 = load_initial_conditions_from_k_rp2x4_total_current(K_rp2x4_i_tot, opt_rp2x4_i_tot);
	%opt_rp2x4_i_tot.x0
    %opt_rp2x4_i_tot.x0(4, 1) = 0;
	%K_rp2x4_i_tot
	opt_rp2x4_i_tot.K = k_rp2x4_i_tot;
	opt_rp2x4_i_tot.ATP = ATP;
	opt_rp2x4_i_tot.ATP_t1 = ATP_t1;
	opt_rp2x4_i_tot.ATP_t2 = ATP_t2;
	opt_rp2x4_i_tot.tspan = [0.0 t_max];
    observable_index = 4;
    %g = 2.416;
    a = - 372;%6.8 / 6.2) * 372;
    ...
    for i=1:1:length(ATP)
        opt_rp2x4_i_tot.ATP = ATP(i);
        my_ATP = ATP(i);
        sol_rp2x4_i_tot(i) = ode15s(@(t, x)(reaction_network_rp2x4_total_current(t, x, opt_rp2x4_i_tot)), opt_rp2x4_i_tot.tspan, opt_rp2x4_i_tot.x0, odeopt);
    end

	tint = linspace(0, t_max, 1000);  % all time points.
	y_rp2x4_i_tot1 = deval(sol_rp2x4_i_tot(1), tint);
    y_rp2x4_i_tot2 = deval(sol_rp2x4_i_tot(2), tint);
    y_rp2x4_i_tot3 = deval(sol_rp2x4_i_tot(3), tint);
    y_rp2x4_i_tot4 = deval(sol_rp2x4_i_tot(4), tint);
    y_rp2x4_i_tot5 = deval(sol_rp2x4_i_tot(5), tint);
    y_rp2x4_i_tot6 = deval(sol_rp2x4_i_tot(6), tint);
    
    steps = 10000;

	fig1 = figure('Name', 'Simulation of total current for P2X4 receptor in human microglia');
	fig1.WindowStyle = 'docked';
	O1 = y_rp2x4_i_tot1(observable_index, :);
    O2 = y_rp2x4_i_tot2(observable_index, :);
    O3 = y_rp2x4_i_tot3(observable_index, :);
    O4 = y_rp2x4_i_tot4(observable_index, :);
    O5 = y_rp2x4_i_tot5(observable_index, :);
    O6 = y_rp2x4_i_tot5(observable_index, :);
	hold on;
	plot(tint, O1 * a, '-ko', 'MarkerIndices', 1:steps:length(O1));
    plot(tint, O2 * a, 'g--', 'MarkerIndices', 1:steps:length(O2));
    ...
    str = {'[C]', '[S]', '[D]', '[O]'};
    for i=1:1:length(opt_rp2x4_i_tot.x0)
       subplot(2, 2, i);
       %str = { sprintf('s%d', i) };
       plot(tint, y_rp2x4_i_tot5(i, :)); xlabel('Time (s)'); ylabel(str(i));
       %xlim([0.0 8.5]);
    end
    return
    
    %{
    % Write 5mM response of the rP2X4 to file
    file = fopen("rP2X4-ATP-5mM-total-current.dat", "w");
    ...
    my_ATP = ATP;
    odeopt = odeset;%('Jacobian', @compute_jacobian_matrix_rp2x4_total_current);
    sol = ode15s(@(t, x)(reaction_network_rp2x4_total_current(t, x, opt_rp2x4_i_tot)), opt_rp2x4_i_tot.tspan, opt_rp2x4_i_tot.x0, odeopt);
	...
	y_rp2x4_i_tot = deval(sol, tint);
    
	O = y_rp2x4_i_tot(observable_index, :);
	hold on;
    ...
    for i=1:1:length(opt_rp2x4_i_tot.x0)
       subplot(4, 2, i);
       %str = { sprintf('s%d', i) };
       plot(tint, y_rp2x4_i_tot(i, :)); xlabel('Time (s)'); ylabel(str(i));
       %xlim([0.0 8.5]);
    end
end
%--------------------%
function [K] = read_last_line()
    filename = "outcmaes-rp2x4-ATP-1.000000e-04-popsize-18-total-currentxmean.dat";
