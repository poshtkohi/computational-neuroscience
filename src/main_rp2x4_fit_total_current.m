...
pop_size = c.PopSize;
%------Read data from file -------%
d = load('data/rP2X4-ATP-0.1mM-total-current.dat');

ATP = 0.1e-3; % unit in mollars
%ATP_t1 = 19.56; ATP_t2 = 19.56 + 30.43; t_max = 70.0;% unit in seconds
ATP_t1 = 19.56; ATP_t2 = 54.25; t_max = 70.0;% unit in seconds
t = d(:, 1);
i_tot = d(:, 2);% * 1e-3; % pA
i_shape = normalize(-i_tot, 'range');% * 6.2 / 6.8;
...
K_init = load('data/initial-guess-k-rp2x4-total-current.dat');
%previous_experiment = '1000';
[K_new, num_added] = load_initial_conditions_from_file_rp2x4_total_current(K_init);
opt.K = K_new;
opt.initial_condition_index = length(K_init) + 1;
opt.x0 = zeros(4, 1);
opt.x0(1, 1) = 1;
opt.x0 = load_initial_conditions_from_k_rp2x4_total_current(opt.K, opt);
%opt.S = sparse(S);
opt.odeopt = odeopt;
opt.tspan = tspan;
opt.tint = tint;
...
ops.SaveFilename = sprintf('variablescmaes-rp2x4-ATP-%d-popsize-%d-total-current.mat', ATP, pop_size);
ops.LogFilenamePrefix = sprintf('outcmaes-rp2x4-ATP-%d-popsize-%d-total-current  % files for output data', ATP, pop_size); 
...
[X, F, COUNT, STOP, OUT, BESTEVER]= cmaes('fhngen_rp2x4_fit_total_current', y0, sigma, ops, opt, w);
