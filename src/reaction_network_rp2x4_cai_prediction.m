...
%------Functions -------%
function [dxdt] = reaction_network_rp2x4_cai_prediction(t, x, opt)
 %-- Simulation Settings --%
    global nT_P2X T;
    k = opt.K;
    ...
    %fprintf('t=%g\tCAi=%gnM\tI_P2X7_CAi=%g\tI_NCX=%g\tI_PMCA=%g\tI_LEAK=%g\n', t, CAi * 1e9, 0.08*i_rp2x4_tot, NCX_Current(CAi)*s_tot, PMCA_Current(CAi)*s_tot, Leak_Current(CAi)*s_tot);
   
    i_cai_tot = -(0.08 * i_rp2x4_tot + NCX_Current(CAi) * s_tot + PMCA_Current(CAi) * s_tot + Leak_Current(CAi) * s_tot);
    ...
    %i_eflux = 0.0004 * CAi;
    %i_cai_tot = 0.1 * i_rp2x4_tot - i_eflux;
    
    dCdt = k(3) * S - k(1) * ATP * C + k(5) * O - k(7) * C;
	dSdt = k(1) * ATP * C + k(4) * O - k(3) * S - k(2) * S - k(6) * S;
	dOdt = k(2) * S - k(4) * O - k(5) * (O^2 + O) + k(7) * C;
    alpha = 1;
