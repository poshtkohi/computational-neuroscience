...
%------Functions -------%
function [I_Na_NCX] = NA_NCX_Current_Fit(k, Vm)
    CAx = 2e-3;     % M
    NAx = 145e-3;   % M
    %%NAi = 8e-3;   % M
    ...
    gamma = k(2); % NCX partition parameter
    n = k(3);
    NAi = k(4) ;
    CAi = k(5);
    %CAi = 45e-9;
    
    beta1 = (NAi / NAx)^n * exp(gamma * F * Vm * R^-1 * T^-1);
...
%%%function [I_NCX] = NCX_Current(NAi, CAi)
...
%%%    g_NCX = 1.99; % NCX conductance; unit: s
%%%    
