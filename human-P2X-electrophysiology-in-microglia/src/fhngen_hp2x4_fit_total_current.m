...
%------Functions -------%
function [err] = fhngen_hp2x4_fit_total_current(x, opt, w_experimental)
    ...
    opt.x0 = load_initial_conditions_from_k_hp2x4_total_current(x, opt);
	...
	sol = ode15s(@(t, x)(reaction_network_hp2x4_total_current(t, x, opt)), tspan, opt.x0, opt.odeopt);
    ...
    %if err < err_prev
        ...
        mm = -max(w);
        plot([opt.ATP_t1 opt.ATP_t2], [mm/2 mm/2], '-k' , 'LineWidth', 2);
        text((opt.ATP_t1 + (opt.ATP_t2 - opt.ATP_t1) /2), mm/2 - 0.07*mm/2, 'ATP');
        xlabel('Time (s)');
        ylabel('I_{tot} (pA)'); 
        %ylabel('Normalised [O]');
        %xlim([0.0 8.5]);
