...
function [dxdt] = reaction_network_p2x_cai_prediction(t, x, opt)
 %-- Simulation Settings --%
    ...
    I_hP2X7_Ca = (0.1 * g_p2x7 + 0.1 * g_p2x7 * opt.SA(opt.SAg_p2x7)) * O1 * (Vm - E_Ca) * 1e3 * 1e-12;
    ...
    g_p2x4 = 6.2;
    I_rP2X4_Na = (0.12 * g_p2x4 + 0.12 * g_p2x4 * opt.SA(opt.SAg_p2x7)) * O2 * (Vm - E_Na) * 1e3 * 1e-12;
    I_rP2X4_Ca = (0.1 * g_p2x4 + 0.1 * g_p2x4 * opt.SA(opt.SAg_p2x7)) * O2 * (Vm - E_Ca) * 1e3 * 1e-12;
    ...
    %ki1_p2x4 = 0.1;
    %ki2_p2x4 = 0.1;
    i1_p2x4 = 1;%exp(-ki1_p2x4 * A);
    i2_p2x4 = 1;%exp(-ki2_p2x4 * A);
    
    k1b_p2x4 = k1b_p2x4 * i1_p2x4;
    k2b_p2x4 = k2b_p2x4 * i2_p2x4;
    
    k4b_p2x4 = k4b_p2x4 * exp(-1000 * O2);
    A = ATP * 1e3;
    k3f_p2x4 = k3f_p2x4 * exp(A);% * exp(A/1e2);
    k3b_p2x4 = k3b_p2x4 * exp(A);% * exp(A/1e2);
    
    dC2dt = k1b_p2x4 * S2 - k1f_p2x4 * ATP * C2 + k4b_p2x4 * D2;
	dS2dt = k3b_p2x4 * D2 + k1f_p2x4 * ATP * C2 + k2b_p2x4 * O2 - k1b_p2x4 * S2 - k2f_p2x4 * ATP * S2 - k3f_p2x4 * S2;
    dD2dt = k3f_p2x4 * S2 - k3b_p2x4 * D2 - k4b_p2x4 * D2;
	dO2dt = k2f_p2x4 * ATP * S2 - k2b_p2x4 * O2;
