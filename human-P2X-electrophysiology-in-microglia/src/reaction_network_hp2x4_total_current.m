...
%------Functions -------%
function [dxdt] = reaction_network_rp2x4_total_current(t, x, opt)
    %-- Simulation Settings --%
    global nT_P2X T;
    k = opt.K;
    ...
    %k3b = k3b * i;
    %k3f = k3f * O;
    %k3f = k3f * ATP * 1e4;%exp(-10*O);
    %k3b = k3b * (ATP * 1e4 + 1e-6)^-1;
    %k3b = k3b * exp(-10*O);
    %i
    %k1b = k1b * exp(-ki * O);
    %k2b = k2b * exp(-ki * O);
	...
	dSdt = k3b * F(D, kd) + k1f * ATP * F(C, kc) + k2b * F(O, ko) - k1b * F(S, ks) - k2f * ATP^0 * F(S, ks) - k3f * F(S, ks);
    dDdt = k3f * F(S, ks) - k3b * F(D, kd) - k4b * D;
	dOdt = k2f * ATP^0 * F(S, ks) - k2b * F(O, ko);
